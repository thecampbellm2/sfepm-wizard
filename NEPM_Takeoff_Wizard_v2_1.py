...
Changes in v2.0:
  ...
  - Rates & Assumptions sheet: Notes column in both RATES and KEY ASSUMPTIONS
    sections now merges across all remaining columns
  - Logo now occupies column A only on all sheets; job name merges B through last

Changes in v1.9:
  - Fixed: python312.dll now reliably bundled — corrected quote-escaping
    bug in build.bat that caused DLL detection to silently fail
  - Build process now automatically adds Windows Defender exclusion
    for the dist folder, preventing scan-lock on freshly built exes
  - Python-level error logging added: crashes write to nepm_wizard_error.log
    alongside the exe for easy diagnosis
...
def title_block(ws, job_name, sheet_label, n_cols=16):
    ...
    LOGO_COLS = 1   # logo occupies column A only; job name merges B through last

    # ── Row 1 — mega header ───────────────────────────────────────────────────
    ws.row_dimensions[1].height = 152

    # Logo area (always column A — no merge needed for single column)
...
def build_rates(wb, scope, brick_types, block_types):
    ...
    for ci, h in enumerate(["Item", "Unit", "Rate ($)", "Notes"], 1):
        hdr_cell(ws, r, ci, h)
    for ec in [5, 6]:  # extend Notes header across unused cols
        ws.cell(r, ec).fill = fill(C_DARK_BLUE); ws.cell(r, ec).border = xborder()
    ...
    for item, unit, note in rate_items:
        ...
        for ec in [5, 6]:  # extend Notes across unused cols
            ws.cell(r, ec).fill = fill(bg); ws.cell(r, ec).border = xborder()
        ws.merge_cells(f"D{r}:F{r}")
        ws.row_dimensions[r].height = 18; r += 1
    ...
    for ec in [4, 5, 6]:  # extend Notes header across unused cols
        ws.cell(r, ec).fill = fill(C_DARK_BLUE); ws.cell(r, ec).border = xborder()
    ...
    for assump, val, note in assumptions:
        ...
        for ec in [4, 5, 6]:  # extend Notes across unused cols
            ws.cell(r, ec).fill = fill(bg); ws.cell(r, ec).border = xborder()
        ws.merge_cells(f"C{r}:F{r}")
        ws.row_dimensions[r].height = 18; r += 1
...
if __name__ == "__main__":
    ...
    # Error log sits alongside the exe (or the script when run as .py)
    _base = (os.path.dirname(sys.executable)
             if getattr(sys, "frozen", False)
             else os.path.dirname(os.path.abspath(__file__)))
    _log  = os.path.join(_base, "nepm_wizard_error.log")
    ...
    try:
        ...
        try:
            messagebox.showerror(
                "Unexpected Error",
                f"The wizard encountered an unexpected error and needs to close.\n\n"
                f"A log has been saved to:\n{_log}\n\n"
