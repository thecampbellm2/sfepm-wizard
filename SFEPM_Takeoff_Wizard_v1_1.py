...
Changes in v1.0 (internal v5):
  ...
  - Dynamic brick/block calculator on Rates & Assumptions sheet
    (bricks/blocks per M2 auto-calculated from entered dimensions)
  - 10 brick/block type slots in wizard (up from 5)
  - Masonry/Demo themes carried forward from v2
"""
...
def check_for_updates():
    """
    Silently checks GitHub for a newer version on startup.
    Only runs when packaged as a PyInstaller exe (sys.frozen = True).
    Fails silently on any network or parsing error — never blocks the app.
    """
    if not getattr(sys, "frozen", False):
        return   # Running as .py script — skip update check
    ...
    try:
        ...
        want = messagebox.askyesno(
            "Update Available — SFEPM Takeoff Wizard",
            f"A new version is available!\n\n"
            f"   Current : {VERSION}\n"
            f"   Latest  : v{latest}\n\n"
            f"Download and install now?\n"
            f"(The wizard will relaunch automatically)"
        )
        _root.destroy()
        ...
        if not done[0]:
            return   # User closed window mid-download
...
def build_brickwork(wb, job_name, brick_types, brick_rates_rows):
    ...
    # Col:  1=Desc  2=BrickType  3=LM  4=Height  5=M2  6=Ded  7=TotM2
    #       8=#Brk  9=Wastage    10=Rate  11=Total  12=Notes
    set_col_widths(ws, [30, 28, 7, 9, 8, 13, 10, 12, 12, 10, 12, 22])
    ...
    # Build a lookup: brick_type_name -> rates_sheet_row for bricks/m2
    type_to_rate_row = {}
    for i, bt in enumerate(brick_types):
        if i < len(brick_rates_rows):
            type_to_rate_row[bt] = brick_rates_rows[i]
    ...
    for r in range(DATA_START, DATA_START + DATA_ROWS):
        ...
        # Formula: SUMIF to find the matching bricks/m2 for this row's brick type
        c = data_cell(ws, r, 8, bg=C_LIGHT_BLUE, fmt="#,##0")
        c.value = (
            f'=IF(OR(G{rs}="",B{rs}=""),"",ROUND(G{rs}*'
            f'IFERROR(INDEX(\'{RATES_SHEET}\'!$E$3:$E$200,'
            f'MATCH(B{rs},\'{RATES_SHEET}\'!$A$3:$A$200,0)),50),0))'
        )

        # Wastage +3%
        c = data_cell(ws, r, 9, bg=C_LIGHT_BLUE, fmt="#,##0")
        c.value = f'=IF(H{rs}="","",ROUND(H{rs}*1.03,0))'

        data_cell(ws, r, 10, bg=bg, fmt="$#,##0.0000")  # Rate ($ per brick — small number)

        # Total = Wastage qty × Rate per brick
        c = data_cell(ws, r, 11, bg=C_LIGHT_BLUE, fmt="$#,##0.00")
        c.value = f'=IF(I{rs}="","",I{rs}*J{rs})'

        data_cell(ws, r, 12, bg=bg)                     # Notes
    ...
    c.alignment = align(h="right")
    for col, fmt in {7: "0.00", 8: "#,##0", 9: "#,##0", 11: "$#,##0.00"}.items():
...
def build_blockwork(wb, job_name, block_data, block_rates_rows):
    ...
    block_types = [d[0] for d in block_data]  # names only for display
    ...
    # Col: 1=Desc 2=BlkType 3=Size 4=LM 5=Ht 6=M2 7=Ded 8=TotM2
    #      9=#Blk 10=Wast 11=CF? 12=CorefM3 13=Steel 14=SBar 15=Cap
    #      16=Rate 17=Total 18=Notes
    set_col_widths(ws, [26, 26, 10, 6, 9, 8, 13, 10, 10, 10,
    ...
    for ci, h in enumerate(HEADERS, 1):
        bg = C_MID_BLUE if ci == 11 else C_DARK_BLUE
    ...
    if block_data:
        ...
        dv_blk = DataValidation(type="list", formula1='"Type"',
                                allow_blank=True, showErrorMessage=False)
    # Size and corefill still available as manual overrides via dropdown
    ...
    dv_cf.sqref  = f"K{DATA_START}:K{DATA_START+DATA_ROWS-1}"

    for r in range(DATA_START, DATA_START + DATA_ROWS):
        bg = row_bg(r); rs = str(r)

        data_cell(ws, r,  1, bg=bg)
        data_cell(ws, r,  2, bg=bg)
        # Col 3: Block Size — auto-populated via VLOOKUP on block type name
        # Falls back to blank if not found; manually overrideable by typing
        ...
        # No. of Blocks — looks up blocks/m2 from Rates sheet
        c = data_cell(ws, r, 9, bg=C_LIGHT_BLUE, fmt="#,##0")
        c.value = (
            f'=IF(H{rs}="","",ROUND(H{rs}*12.5,0))'
        )

        # Wastage +3%
        c = data_cell(ws, r, 10, bg=C_LIGHT_BLUE, fmt="#,##0")
        c.value = f'=IF(I{rs}="","",ROUND(I{rs}*1.03,0))'

        # Corefilled? — auto-populated from wizard via VLOOKUP; manually overrideable
        c = data_cell(ws, r, 11, bg=C_LIGHT_BLUE)
        ...
        c.value = (
            f'=IF(K{rs}<>"Yes","",'
            f'IF(C{rs}="290mm",ROUND(I{rs}/80,2),'
            f'IF(C{rs}="190mm",ROUND(I{rs}/110,2),'
            f'IF(C{rs}="140mm",ROUND(I{rs}/130,2),"N/A (90mm)"))))'
        )

        # Steel: 10 kg/m² corefilled
        c = data_cell(ws, r, 13, bg=C_LIGHT_BLUE, fmt="#,##0.0")
        c.value = f'=IF(K{rs}<>"Yes","",IF(C{rs}="90mm","N/A",ROUND(H{rs}*10,1)))'
        ...
        data_cell(ws, r, 16, bg=bg, fmt="$#,##0.0000")  # Rate ($ per block)

        # Total = Wastage qty × Rate per block
        c = data_cell(ws, r, 17, bg=C_LIGHT_BLUE, fmt="$#,##0.00")
        c.value = f'=IF(J{rs}="","",J{rs}*P{rs})'

        data_cell(ws, r, 18, bg=bg)
        ws.row_dimensions[r].height = 18
...
def build_george_summary(wb, job_name, scope, brick_types, block_types):
    ...
    title_block(ws, job_name, "GEORGE SUMMARY", 12)
    set_col_widths(ws, [35, 11, 11, 11, 11, 11, 11, 11, 12, 16, 16, 20])
    ...
    if scope in ("brickwork", "both"):
        ...
        for ci, h in enumerate(["Brick Type", "Total M2", "No. of Bricks",
            ...
            bg = "2E6B3E" if ci in (8, 9) else C_MID_BLUE
        ...
        for bt in brick_types:
            ...
            # SUMIF on col G (Total M2 col 7), H (Bricks col 8), I (Wastage col 9)
            ws.cell(r, 2).value = f'=IFERROR(SUMIF(Brickwork!B{DS}:B{DE},A{r},Brickwork!G{DS}:G{DE}),0)'
            ws.cell(r, 3).value = f'=IFERROR(SUMIF(Brickwork!B{DS}:B{DE},A{r},Brickwork!H{DS}:H{DE}),0)'
            ws.cell(r, 4).value = f'=IFERROR(SUMIF(Brickwork!B{DS}:B{DE},A{r},Brickwork!I{DS}:I{DE}),0)'
            # Total = Wastage qty (col 4) × Rate (col 5, manual)
            ws.cell(r, 6).value = f'=IF(E{r}="","",D{r}*E{r})'
            fmts = {2:"0.00", 3:"#,##0", 4:"#,##0", 6:"$#,##0.00"}
            ...
            # ── Sanity check cols ──────────────────────────────────────────
            # Col 8: reverse-engineer M2 from bricks (÷50), compare to Total M2
            c = ws.cell(r, 8)
            ...
            # Col 9: reverse-engineer bricks from wastage (÷1.03), compare to no. of bricks
            c = ws.cell(r, 9)
        ...
        for ci in range(2, 13): ws.cell(r, ci).fill = fill(C_MID_BLUE)
    ...
    if scope in ("blockwork", "both"):
        ...
        for ci, h in enumerate(["Block Type", "Total M2", "No. of Blocks",
            ...
            bg = "2E6B3E" if ci in (11, 12) else C_DARK_BLUE
            hdr_cell(ws, r, ci, h, bg=bg)
        ws.row_dimensions[r].height = 30; r += 1
        ...
        for blk in block_types:
            ...
            ws.cell(r, 1).font = xfont()
            ws.cell(r, 1).alignment = align(h="left")
            # Blockwork cols: H=TotM2(8), I=Blocks(9), J=Wastage(10),
            #                 L=Corefill(12), M=Steel(13), N=StarterBars(14)
            ws.cell(r, 2).value = f'=IFERROR(SUMIF(Blockwork!B{DS}:B{DE},A{r},Blockwork!H{DS}:H{DE}),0)'
            ws.cell(r, 3).value = f'=IFERROR(SUMIF(Blockwork!B{DS}:B{DE},A{r},Blockwork!I{DS}:I{DE}),0)'
            ws.cell(r, 4).value = f'=IFERROR(SUMIF(Blockwork!B{DS}:B{DE},A{r},Blockwork!J{DS}:J{DE}),0)'
            ws.cell(r, 5).value = f'=IFERROR(SUMIF(Blockwork!B{DS}:B{DE},A{r},Blockwork!L{DS}:L{DE}),0)'
            ws.cell(r, 6).value = f'=IFERROR(SUMIF(Blockwork!B{DS}:B{DE},A{r},Blockwork!M{DS}:M{DE}),0)'
            ws.cell(r, 7).value = f'=IFERROR(SUMIF(Blockwork!B{DS}:B{DE},A{r},Blockwork!N{DS}:N{DE}),0)'
            # Total = Wastage qty (col 4) × Rate (col 8, manual)
            ...
            for col in [8, 10]:
                ...
                c.fill = fill(row_bg(r)); c.border = xborder(); c.font = xfont()
            # ── Sanity check cols ──────────────────────────────────────────
            # Col 11: reverse-engineer M2 from blocks (÷12.5), compare to Total M2
            c = ws.cell(r, 11)
            ...
            c.fill = fill(row_bg(r)); c.border = xborder()
            c.font = xfont(sz=9); c.alignment = align(h="center", wrap=False)
            # Col 12: reverse-engineer blocks from wastage (÷1.03), compare to no. of blocks
            c = ws.cell(r, 12)
            c.value = (f'=IF(D{r}=0,"—",IF(ABS(ROUND(D{r}/1.03,0)-C{r})<=1,'
        ...
        c.font = xfont(bold=True, c=C_AMBER); c.fill = fill(C_DARK_BLUE)
        c.number_format = "$#,##0.00"
        for ci in range(2, 11): ws.cell(r, ci).fill = fill(C_DARK_BLUE)
        ws.row_dimensions[r].height = 22
    ...
    for ci in range(1, 11): ws.cell(r, ci).fill = fill(C_DARK_BLUE)
